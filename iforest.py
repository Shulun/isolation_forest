...
class IsolationTreeEnsemble:
    ...
    def fit(self, X:np.ndarray, improved=False):
        ...
        for _ in range(self.n_trees):
            ...
            itree = IsolationTree(self.l)
            self.trees.append(itree.fit(X[ix]))
...
class IsolationTree:
    ...
    def fit(self, X:np.ndarray, improved=False):
        ...
        if self.current_height >= self.height_limit or len(X) <= 1:
            ...
            if improved:
                n = 3
                q_lst = random.sample(range(X.shape[1]), n)
                p_lst = np.stack([np.random.uniform(X[:,qi].min(), X[:,qi].max(), n) for qi in q_lst])
                dist_mat = np.zeros((n, n))
                for i in range(n):
                    for j in range(n):
                        dist_mat[i, j] = min(sum(X[:, q_lst[i]] < p_lst[i, j]), 
                                             sum(X[:, q_lst[i]] >= p_lst[i, j])) 
                (x, y) = np.where(dist_mat == np.min(dist_mat))
                q, p = q_lst[x[0]], float(p_lst[x[0], y[0]])
                
            tree_left = self.fit(X[X[:, q] < p])
            tree_right = self.fit(X[X[:, q] >= p])

            self.current_height += 1
            n_nodes = self.count_nodes(tree_left) + self.count_nodes(tree_right)
            self.root = DecisionNode(size=len(X), splitAttr=q, splitVal=p, n_nodes=n_nodes,
...
class DecisionNode:
    def __init__(self, size, splitAttr, splitVal, left, right, n_nodes):
        self.size = size
        self.left = left
        self.right = right
...
class LeafNode:
    def __init__(self, size, left=None, right=None, n_nodes=1):
        self.size = size   
        self.left = left
        self.right = right
        self.n_nodes = n_nodes
